...
function reengraveForInstrument(): void {
  ...
  clearHighlights();
  resetFollow();
  const visualObj = ABCJS.renderAbc(sheetMusicEl, effectiveAbc(state.currentAbc), {
    responsive: "resize",
    add_classes: true,
  });
...
function applySettings(prepare?: () => void): Promise<void> {
  // `prepare` runs inside the link, right before the re-prime reads
  // `state.visualObj` (the instrument re-engrave, #25).
  const next = applySettingsChain.then(() => {
    prepare?.();
